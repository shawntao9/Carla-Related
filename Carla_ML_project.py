...
class CarEnv:
    ...
    def reset(self):
        ...
        self.colsensor = self.world.spawn_actor(col_sensor, transform, attach_to=self.vehicle)
...
class DQNAgent:
    ...
    def train(self):
        ...
        with self.graph.as_default():
            self.model.fit(np.array(X)/255, np.array(), batch_size=TRANING_BATCH_SIZE, verbose=0, shuffle=False, callbacks=[self.tensorboard] if log_this_step else None)
        
        if log_this_step:
            self.target_upate_counter += 1

        if self.target_upate_counter > UPDATE_TARGET_EVERY:
            self.target_model.set_weights(self.model.get_weights())
...
#     sensor = world.spawn_actor(cam_bp, cam_spawn_point, attach_to=vehicle)
